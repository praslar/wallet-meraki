...
func (h *UserHandler) CreateToken(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.tokenService.CreateToken(requestToken.Symbol, requestToken.TotalSupply); err != nil {
		logrus.Errorf("Failed create user: %v", err.Error())
		w.WriteHeader(http.StatusInternalServerError)
		err := json.NewEncoder(w).Encode(map[string]interface{}{
			"error": err.Error(),
...
func (h *UserHandler) UpdateToken(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.tokenService.UpdateToken(requestToken.Address, requestToken.Symbol); err != nil {
		logrus.Errorf("Failed create user: %v", err.Error())
		w.WriteHeader(http.StatusInternalServerError)
		err := json.NewEncoder(w).Encode(map[string]interface{}{
			"error": err.Error(),
...
func (h *UserHandler) DeleteToken(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.tokenService.DeleteToken(requestToken.Address, requestToken.Symbol); err != nil {
		logrus.Errorf("Failed create user: %v", err.Error())
		w.WriteHeader(http.StatusInternalServerError)
		err := json.NewEncoder(w).Encode(map[string]interface{}{
			"error": err.Error(),

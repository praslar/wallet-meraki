...
func (s *AuthService) GenJWTToken(userID uuid.UUID, key string) (string, error) {
...
func (s *AuthService) ValidJWTTokenXuser(token string, requiredID uuid.UUID) error {
	claims := &Claims{}
	// Parse the JWT string and store the result in `claims`.
	// Note that we are passing the key in this method as well. This method will return an error
	// if the token is invalid (if it has expired according to the expiry time we set on sign in),
	...
	if !tkn.Valid {
		return fmt.Errorf("unauthorized valid")
	}
	_, err = s.userRepo.GetUserByID(claims.XUserID)
	if err != nil {
		return fmt.Errorf("fail:s.userRepo.GetUserByID(claims.XUserID)")
